...
function AnimatedStat({ value, label, icon, inView }: Stat & { inView: boolean }) {
  ...
  return (
    <div className="relative group text-center p-4 sm:p-5 rounded-2xl border border-neutral-100 dark:border-neutral-800 bg-white dark:bg-neutral-900 hover:border-neutral-200 dark:hover:border-neutral-700 hover:shadow-md transition-all duration-300 h-full flex flex-col items-center justify-center">
      {Icon && (
        <div className="mb-2 flex h-9 w-9 items-center justify-center rounded-xl bg-neutral-100 dark:bg-neutral-800 group-hover:bg-neutral-900 dark:group-hover:bg-white transition-colors duration-300">
          <Icon className="h-4 w-4 text-neutral-600 dark:text-neutral-400 group-hover:text-white dark:group-hover:text-neutral-900 transition-colors duration-300" />
        </div>
      )}
      ...
      <p className="text-[11px] sm:text-xs text-neutral-500 dark:text-neutral-400 mt-1 font-medium leading-tight">{label}</p>
    </div>
  );
}
  ...
  return (
    <div ref={ref} className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 sm:gap-4">
      {stats.map((stat, i) => (
        <div
          key={stat.label}
          className={`transition-all duration-700 ease-out ${i >= 4 ? "hidden sm:block" : ""}`}
          style={{
            opacity: inView ? 1 : 0,
