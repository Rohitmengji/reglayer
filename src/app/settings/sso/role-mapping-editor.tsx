...
export function RoleMappingEditor({ connectionId }: { connectionId: string }) {
  ...
  async function save() {
    ...
    if (new Set(groups).size !== groups.length) {
      toast.error("Duplicate IdP group names (case-insensitive)");
      return;
    }
    setSaving(true);
    const toastId = toast.loading("Saving role mappings…");
    try {
      ...
      if (!res.ok) {
        toast.error(data.error || "Failed to save role mappings", { id: toastId });
        return;
      }
      setRows((data.mappings || cleaned).map((m: RoleMappingView) => ({ key: String(keyCounter.current++), idpGroup: m.idpGroup, role: m.role })));
      toast.success("Role mappings saved", { id: toastId });
    } finally {
      setSaving(false);
    }
  }

  if (loading) return <p className="text-xs text-neutral-500 py-2">Loading role mappings…</p>;
  if (loadError) return <p className="text-xs text-red-600 py-2">Couldn&apos;t load role mappings.</p>;

  return (
    <div className="space-y-2">
      <p className="text-xs text-neutral-500 dark:text-neutral-400">
        Map an IdP group claim to a workspace role applied on each SSO login. The highest role assignable here is Admin.
      </p>
      {rows.length === 0 && <p className="text-xs text-neutral-400">No group mappings — members get the connection&apos;s default role.</p>}
      {rows.map((row, i) => (
        <div key={row.key} className="flex items-center gap-2">
          <input
            type="text"
            placeholder="IdP group (e.g. sso-admins)"
            value={row.idpGroup}
