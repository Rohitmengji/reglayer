...
<head>
  ...
  <title>ACR - ${doc.metadata.productName}</title>
  <style>
...
<body>
  ...
  <h2>${doc.metadata.productName}</h2>
  <div class="metadata">
    <p><strong>Report Date:</strong> ${doc.metadata.reportDate} | <strong>Standard:</strong> ${doc.metadata.standard}</p>
    <p><strong>Vendor:</strong> ${doc.metadata.vendorName}${doc.metadata.vendorContact ? ` (${doc.metadata.vendorContact})` : ''}</p>
    ${doc.metadata.productDescription ? `<p><strong>Description:</strong> ${doc.metadata.productDescription}</p>` : ''}
  </div>
  ...
  <p style="margin-top: 2rem; color: #9ca3af; font-size: 0.8rem; text-align: center;">
    Generated by RegLayer — Automated Accessibility Compliance
  </p>
</body>
</html>`;
}
