...
Built on **BoxyHQ Jackson** bridged into NextAuth v4 (JWT, no adapter). Per-tenant IdP connections; employees JIT-provision into their workspace. **Feature-flagged OFF by default** (`SSO_ENABLED` env + per-connection `rolloutStage`); pricing stays "coming soon" until a real-IdP round-trip is verified. See `docs/architecture/SSO_REVIEW.md` for the adversarial review (the embedded→service swap path, revocation, verified-domain uniqueness). **Admin setup guides + the exact SP config values** (ACS URL, Entity ID, attributes, group→role): `docs/sso/` (Okta, Entra, Google Workspace, troubleshooting); local browser-test runbook: `docs/SSO_TESTING.md`. Dev helpers: `scripts/seed-sso-dev.ts` (one-command working mocksaml connection + verified `example.com`; `--remove` to undo) and `scripts/load/sso-discovery.k6.js` (k6 load test #36). Both the login-page copy (`login.sso*`) and the `/settings/sso` admin UI (`sso.*`) are i18n'd across all 7 locales (only short enum dropdown labels — SAML/OIDC, GA/Beta, role names — stay English). Adding any `en.ts` key REQUIRES adding it to de/es/fr/it/nl/pt too (`i18n-parity.test.ts` enforces it).

**Dependency posture:** `@boxyhq/saml-jackson` is a **devDependency** — its tree carries high-severity, partly-unfixable vulns (`@grpc/grpc-js`/OpenTelemetry via `@boxyhq/metrics`) that fail `npm audit --omit=dev --audit-level=high`. The embedded backend is for **dev/test + the verified API contract**; **production GA uses the SERVICE backend** (a standalone, BoxyHQ-maintained Jackson over HTTPS) so that tree never enters our app's production deps. The `SsoBackend` seam makes the swap one factory line. **Deploy kit:** `deploy/jackson/docker-compose.yml` + `.env.example`, runbook `docs/sso/service-backend.md`, and `scripts/smoke-sso-service.ts` (verifies `JacksonServiceBackend` against a live Jackson end-to-end).
...
- **Admin connection management** — `/api/sso/connections` (GET list, POST create+register-with-Jackson), `/api/sso/connections/[id]` (GET detail incl. domains+mappings, PATCH rollout/enforcement/enable-disable, DELETE soft-delete+release-domains), `/api/sso/connections/[id]/domains` (POST claim → DNS TXT token), `/api/sso/domains/[id]/verify` (POST → resolve TXT, claim `VerifiedDomain` under a `SELECT … FOR UPDATE` lock so a concurrent connection-delete can't orphan it, flip to VERIFIED), `/api/sso/connections/[id]/role-mappings` (GET + PUT replace-all, atomic under the same lock — IdP group→role #25, capped at ADMIN, never OWNER; `idpGroup` lowercased so matching is case-insensitive end-to-end with `resolveProvisionedRole`). All gated by `requireSsoAdmin` (`src/lib/sso/admin-guard.ts` — `sso.manage` permission [new in `rbac.ts`] + Enterprise `sso` feature + rate limit), workspace-scoped (cross-workspace ⇒ 404). Every change writes `recordSsoAudit` (`src/lib/sso/audit.ts`, review #32). (Attribute-mapping endpoints deferred: `User` only has `name`, so mapping `department`/`title`/… would be hollow until those columns exist.)
- **Admin UI** — `src/app/settings/sso/` (`page.tsx` list + add-connection form; `connection-card.tsx` per-connection status/rollout/enable-disable/delete + domain add→TXT→verify; `role-mapping-editor.tsx` group→role replace-all). `"use client"` over the APIs above, matching the `team/page.tsx` conventions (`AppShell`/`Card`, plain `fetch`+`useEffect`, `sonner` toasts, `ModernSelect`/`ConfirmDialog`). Wrapped in `FeatureGate feature="sso"` (Enterprise gate); the API re-enforces role (non-admin ⇒ "admins only" panel). English strings (not i18n yet — follow-up).
- **Cert/health monitoring + self-healing** (`src/lib/sso/cert-health.ts` + `health.ts`, #21/#22/#43) — pure `parseCertNotAfterFromSamlMetadata` (stored on `certificateExpiresAt` at create), `evaluateCertHealth`, `shouldAlertAt` (day-marks 90/60/30/14/7/1). `runSsoHealthChecks()` is the sole `healthStatus` authority: once-per-UTC-day per connection, it **actively probes** the stored `metadataUrl`/`oidcDiscoveryUrl` (SSRF-guarded `safeFetch` that re-validates every redirect hop), refreshing the cert expiry or marking `INVALID_METADATA`/`VALIDATION_FAILED`; updates `healthStatus`/`lastValidatedAt`; emails OWNER/ADMINs at cert day-marks and once on transition into a broken state (best-effort, `isEmailConfigured` guard). Run by `/api/cron/sso-health` (Bearer `CRON_SECRET`) AND piggybacked on `/api/cron/run-schedules` (Hobby single-cron; isolated try/catch → can't destabilize the scan cron; no 2nd vercel.json entry → no double-run). Partial sweeps resume next run. Connection-card shows the cert expiry date.
- **Enforcement + break-glass** (`src/lib/sso/enforcement.ts`, #24/#23) — pure `evaluateEnforcement()`; the `signIn` callback (gated on `SSO_ENABLED`, **fail-open**) blocks password/Google login when a user's verified domain is governed by a LIVE connection with policy ≠ OPTIONAL. Break-glass: workspace OWNER, master admin, and `ServiceAccount.exemptFromSsoEnforcement` always bypass (logged). `getEnforcementForEmail` (`resolve.ts`) only returns a policy for live connections so SSO can never be required where it isn't available. Login page maps the `AccessDenied` result to an SSO hint; connection-card exposes Optional/Required.
