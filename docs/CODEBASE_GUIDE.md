...
- **Cert/health monitoring** (`src/lib/sso/cert-health.ts` + `health.ts`, #21/#22/#43) — pure `parseCertNotAfterFromSamlMetadata` (stored on `certificateExpiresAt` at connection-create, SAML only), `evaluateCertHealth`, `shouldAlertAt` (fixed day-marks 90/60/30/14/7/1). `runSsoHealthChecks()` updates `healthStatus`/`lastValidatedAt` (only cert-derived statuses) + emails OWNER/ADMINs (best-effort, `isEmailConfigured` guard). Run by `/api/cron/sso-health` (Bearer `CRON_SECRET`) AND piggybacked on `/api/cron/run-schedules` so it runs on Vercel Hobby's single cron (no second vercel.json entry → no double-email). Connection-card shows the cert expiry date.
